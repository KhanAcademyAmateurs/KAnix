/**
 * KAnix v0.1.8.8d#3 (do NOT use)
 * -------------
 * @CHANGES: Fixed bugs involving horrible handling of misnamed directories & overwriting
 * @TOFIX:   Many bugs involving initiation of misnamed directories
 *           The bug involving cd-ing into files?! And seeing their internal properties?!!!
*/
...
var read = function (p) {
    ...
    return o instanceof File ? o.read() : o instanceof Directory ? 3 : o;
};
...
var chmod = function (p, m) {
    var o = getPath(path(p));
    return typeof o === "number" ? o : o.chmod(m);
};
// mkdir directory, self explanatory. If not allowed, return 1, if path invalid, return 2, if exists, return 3.
var mkdir = function (p) {
    var a = getPath(path(p).slice(0, -1));
    
    if (typeof o === "number") {
        return a;
    } else if (getPath(path(p))) {
        return 3;
    }
